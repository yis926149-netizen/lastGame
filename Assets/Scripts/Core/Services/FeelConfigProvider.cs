...
//         CardController/TalentCardSelectionUI/CardDragWorldPreviewController），采用静态 Configure 模式（同 BattleFormulaRule）。
//****************************************
public static class FeelConfigProvider
{
    ...
    // 卡牌拖拽：单阶段卡牌手感（§六）。阈值以 Canvas 参考高度为基准。
    ...
    // 卡牌拖拽世界空间预览（改造计划 §7.1）。
    public static float CardDragPreviewHoverHeight => Config.cardDragPreviewHoverHeight;
    ...
    public static float CardDragLinkWidth => Config.cardDragLinkWidth;
}
