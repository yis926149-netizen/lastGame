Shader "Custom/MountainLowPoly_Fog_Transition"
{
    // 【程序化山脉-阶段 5.4】MountainLowPoly_Fog 的动画专用变体（决策 ⑥/㉖/㉛）。
    // 完整复制稳定版山体 Shader 契约：Triplanar（4.3）、离散 face tier + ridgeKey01 偏移（4.4）、
    // 片元导数平面法线 + 逐面顶点法线光照（4.5）、FogBlend 世界 XZ 掩码（4.6）、Cull Off 双面。
    // 与稳定版的全部差异（必须与 TerrainBase_Fog_Transition 同一演化模式）：
    //   ① Properties 增加 _ChunkProgress / _ChunkAnimBaseY / _ChunkAnimRiseHeight；
    //   ② surf 开头 keep-below clip（与手动 ShadowCaster pass 同一平面）；
    //   ③ ShadowCaster 传 worldPosY 并按同一 clip 平面裁切（阴影几何与可见几何一致）。
    // 与稳定版相同：顶点动画由 C# 侧 SetChunkAnimationProgress 逐帧写 mesh.vertices，
    // vert 禁止读取 UV2/UV3 位移（避免 CPU + Shader 双重动画）。
    // 仅在动画期间由 ChunkMapRenderer 按 Chunk 切换使用；稳定渲染永不加载本 Shader。
    // 山体槽 UV0 数据契约（权威定义见 MountainMaterialContract）：UV0.x=ridgeKey01、
    // UV0.y=faceTier 编码 (tier+0.5)/3；本 Shader 只把 UV0 当数据通道读取。
    Properties
    {
    ...
    SubShader
    {
        ...
        CGPROGRAM
        // 不写 addshadow：末尾手动 ShadowCaster pass 与 surf 执行同一 clip 平面，
        ...
        struct Input
        {
            ...
            float3 worldPos;     // Triplanar 采样 + 片元导数平面法线 + keep-below clip 平面
            float3 worldNormal;  // 仅用于叉乘法线朝向校准与退化面兜底（不直接参与光照/权重）
            // 与稳定态一致：读取 worldNormal 且写 o.Normal 时必须提供 WorldNormalVector 内部数据。
            INTERNAL_DATA
        ...
        void vert(inout appdata_full v, out Input o)
        {
            ...
            // 顶点动画在 C# 侧（SetChunkAnimationProgress），vert 禁止读取 UV2/UV3 位移。
        ...
        {
            // 【顶出方案】keep-below clip 平面（与 TerrainBase_Fog_Transition 同一契约）：
        ...
        // 【顶出方案】手动 ShadowCaster pass：与 surf 同一 clip 平面 + 同一 CPU 已变形顶点
        // （vert 不读 UV2/UV3），保证阴影几何与可见几何一致。
        Pass
        {
    ...
    // 与稳定版一致：回退 Standard 而非 Legacy Diffuse
    FallBack "Standard"
}
