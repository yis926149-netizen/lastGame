"""向「表现配置」工作表追加卡牌拖拽模型预览的 10 个数值列（一次性迁移脚本）。
...
注入：只在包内两个部件上做文本级最小改动——
  ...
  * xl/sharedStrings.xml       追加 20 条字符串（10 个列名 + 10 条中文说明）
  * xl/worksheets/sheetNN.xml  在表头行/说明行/数据行末尾各追加 10 个单元格

其余条目（样式、主题、docProps、其他工作表）按原字节复制，不做任何解析或重排，
避免 XML 往返丢失命名空间声明与 WPS 私有扩展。

用法（幂等：列已存在时直接返回，不做修改）
    python Tools/ConfigExporter/scripts/add_feel_carddrag_columns.py
...
NEW_COLUMNS = [
    ...
    ("cardDragModelFadeIn", "模型淡入占 modelProgress 的比例区间（0~1）", "0.2"),
    ("cardDragPreviewRTSize", "模型预览 RenderTexture 边长（像素，正方形）", "512"),
    ("cardDragPreviewWindowSize", "模型预览窗口边长（Canvas 参考单位）", "512"),
    ("cardDragPreviewCameraDistance", "预览正交相机与模型的距离（世界单位）", "10"),
    ("cardDragPreviewPadding", "预览取景留白系数（正交尺寸 = 模型半径 × 本系数）", "1.25"),
]
...
def main():
    ...
    new_string_cells = 2 * len(NEW_COLUMNS)   # 表头 10 + 说明行 10
