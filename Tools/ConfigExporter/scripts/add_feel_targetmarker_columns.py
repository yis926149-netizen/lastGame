"""向「表现配置」工作表追加卡牌拖拽落点提示的 3 个数值列（一次性迁移脚本）。
...
用法（幂等：列已存在时直接返回，不做修改）
    python Tools/ConfigExporter/scripts/add_feel_targetmarker_columns.py
...
NEW_COLUMNS = [
    ("cardDragTargetIconHeight", "落点图标相对地块中心的抬高（世界单位，须高于模型头顶）", "3.5"),
    ("cardDragTargetIconScale", "落点图标世界缩放基准", "1.0"),
    ("cardDragLinkWidth", "连线宽度（Canvas 参考单位，第二批连线使用）", "6.0"),
]
