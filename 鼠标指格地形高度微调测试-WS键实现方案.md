# 鼠标指格地形高度微调测试（W/S 键）实现方案

> 定位：**动态地图能力测试追加项**——鼠标指针指示的地块按 W 地形高度永久 +1、按 S 永久 -1，用于人工构造各种极端地形情况（超高、负高、水位跨界、单位脚下变水等）。
> 日期：2026-08-05。
...
| 项 | 内容 |
|---|---|
| 输入 | 鼠标悬停地图地块（青色高亮指示）；按住 W 高度逐级 +1、按住 S 逐级 -1（单按立即生效，按住 10Hz 连发） |
| 语义 | **永久**修改逻辑 `Height`（经 `MapMutationService` 事务管线，非直接写字段），动画结束后不回弹 |
| 范围 | 全图任意格：**不跳过水域格**（与波浪测试相反）——故意让 ±1 跨越水位线触发水陆双向重置，这正是极端情况测试点 |
| 验收 | 1) 指针格有高亮且随指针移动刷新；2) 按 W/S 单格高度单调 ±1，mesh 即时重建；3) 水域↔陆地跨界时河流/海岸/movementCost 按 §8 双向重置正确；4) 连续按住无事务嵌套异常、无锁残留；5) 仅编辑器/开发构建可用（Release 不可见） |
...
| `IInputService`（GetKey/GetKeyDown/IsPointerOverUI） | `Assets/Scripts/Core/Interfaces/IInputService.cs` | W/S 键盘 + UI 阻挡检测 |
...
**键位冲突检查**：全代码库无 `KeyCode.W/S` 被占用（`grep KeyCode.(W|S)` 无命中），W/S 可安全使用。
...
- **高亮节流（重要）**：`HexHighlightRenderer.SetHighlightedCells` 每次调用都会 `RebuildChannel` 重建 mesh（HexHighlightRenderer.cs:99-140）。虽然单格重建很便宜，仍应**只在指针格变化时**调用，避免每帧无谓重建。实现：缓存 `_hoveredCell` + 上次屏幕坐标，格不变则跳过。

### 3.2 触发方式：单按立即 + 按住 10Hz 连发
"测极端情况"需要快速连续堆高，只做 `GetKeyDown` 单次 +1 太慢。方案：
```
按住 W/S 且（按键瞬间 或 距上次提交 ≥ 0.1s）→ 提交一次
```
- 按下瞬间 `GetKeyDown` 为真 → 立即响应（手感）；
- 之后 `GetKey` 持续为真 → 每 0.1s 提交一次（10Hz，60 帧/秒×0.1s=6 帧一次，避免每秒 60 次提交）；
- 两个键同按取后判定者（W 优先），避免同帧双提交。
...
- 水域格 W 升过 `WaterLevel=1` → `ApplyPatch` 水→陆双向重置：`HexType=NoRiver`、清河流四字段、`isCoast=false`、`movementCost=1`（MapMutationService.cs:480-493）；
- 陆地格 S 降过水位线 → 陆→水：`movementCost=∞`（:494-499），随后 Commit 的 `EjectUnitsFromImpassableCells` 会把格上单位弹射出去——**这是最值得测的极端联动**（把单位脚下格按成水，观察弹射与落点选择）。
...
### 3.6 UI 阻挡
`IInputService.IsPointerOverUI()` 为真时不取格、不高亮、不响应 W/S（与卡牌拖拽同口径，避免隔着 UI 面板改地形）。

### 3.7 与波浪测试（V 键）并行
两者不冲突：波浪是纯视觉脉冲、逻辑数据已恢复原值（MapWaveTestController.cs:173-183），W/S 改的是逻辑数据；若同格动画期间提交，管线会自动 `ForceCompleteConflicting` 并广播 `Cancelled` 阶段（使用报告 §六-8），行为可观察但无害。
...
// 鼠标悬停地图地块（Selection 通道青色高亮指示）；按住 W 高度永久 +1、按住 S 永久 -1：
...
public class MapHeightEditTestController : ITickable
{
    ...
    public void Tick()
    {
        ...
        bool holdingW = _input.GetKey(KeyCode.W);
        bool holdingS = _input.GetKey(KeyCode.S);
        if (!holdingW && !holdingS) return;

        // 按下瞬间立即提交；随后按 CommitIntervalSeconds 连发
        bool trigger = _input.GetKeyDown(KeyCode.W) || _input.GetKeyDown(KeyCode.S) ||
                       Time.realtimeSinceStartup >= _nextCommitTime;
        if (!trigger) return;

        ApplyHeight(_hoveredCell, holdingW ? +Step : -Step);
    ...
    /// <summary>单格高度 ±Step 的完整事务（读的是 cell 最新 Height，连发天然累加）。</summary>
...
```csharp
// 【动态地图-能力测试】鼠标指格地形高度微调测试（W/S 键单格 ±1 永久修改，2026-08-05）。
// 仅编辑器/开发构建绑定——Release 构建中 W/S 键不得修改地形（与波浪测试同评审口径）。
...
| # | 操作 | 预期观察（联动点） |
|---|---|---|
| 1 | 陆地格连按 W 至 10+ 级 | 3 子 Mesh 分桶切换（`ClassifyHeight`）、雾面/网格线跟随高度、无破面 |
| 2 | 连按 S 至负高度 | 水面判定恒真（`IsWater(负)=true`）自动转水域；mesh 沉入水下 |
| 3 | 水域格 W 跨过水位线（1→2） | 水→陆双向重置：河流四字段清空、`isCoast=false`、`movementCost=1`（MapMutationService.cs:480-493） |
| 4 | 陆地格 S 跨过水位线（2→1） | 陆→水：`movementCost=∞`；若格上有单位 → `EjectUnitsFromImpassableCells` 弹射（含落点选取、找不到落点释放占用 + 警告） |
...
| 8 | 按住 W 不放 30 秒 | 300 次提交无事务嵌套/锁残留；脏 Chunk 持续重建稳定；开 `EnableCommitLogging` 观察每笔日志 |
| 9 | 指针快速扫过地图 | 高亮跟随且无每帧重建（节流生效）；移出地图/移到 UI 上高亮消失 |
| 10 | 波浪测试（V 键）进行中按 W/S | 同格动画冲突走 `ForceCompleteConflicting` + `Cancelled` 阶段广播（使用报告 §六-8），无死锁 |
...
1. 编辑器打开主场景 → Play；
2. 指针悬停陆地格 → 青色高亮出现；按一下 W → 格升高一级（日志 `[MapHeightEdit] 格 N 高度=X`）；按住 W → 10Hz 连续升高；
3. 按 S 回落；把格按回水面以下 → 观察变水域、格上单位（若有）弹射；
4. 水域格按 W 升出水面 → 观察河流/海岸重置；
5. 开 `MapMutationDiagnostics.EnableCommitLogging = true` 观察每笔提交耗时与脏 Chunk 数；
6. 连续按住 W/S 各 30 秒 → 无异常、无卡顿、`MapInteractionGate` 无锁残留（`IsLocked` 查询）；
